...
TipoRet DIR(Sistema &s, Cadena parametro)
{
	...
	if (strcmp(parametro, "/s") == 0)
	{
		if (auxsis == NULL)
		{
			// Caso base: el sistema actual es NULL, termina la recursiÃ³n.
			return OK;
		}
		Pila pil = NULL;
		...
		else if (auxsis->pH != NULL)
		{
			...
			if (auxsis->sH != NULL)
			{
				while (auxsis->sH != NULL)
				{
					apilar(pil, crearNodoPila(auxsis->sH->nombre));
					DIR(auxsis->sH,"/s");
					cout << endl;
					mostrarP(pil);
					auxsis = auxsis->sH;
	...
	TipoRet MOVE(Sistema & s, Cadena nombre, Cadena directorioDestino)
	{
		...
		//			while (auxsis->sH != NULL && strcmp(auxsis->sH->nombre, nombreDirectorio) != 0)
		//			{
		//				auxsis = auxsis->sH;
		//			}
		//
		...
		//		else
		//		{
		//			cout << "No hay subdirectorios en este directorio" << endl;
		//		}
		return NO_IMPLEMENTADA;
	}
